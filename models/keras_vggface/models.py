...
def VGG16(include_top=True, weights='vggface',
    ...
    # Block 1
    x = Conv2D(64, (3, 3), use_bias=use_bias, activation='relu', padding='same', name='conv1_1')(
        img_input)
    x = Conv2D(64, (3, 3), use_bias=use_bias, activation='relu', padding='same', name='conv1_2')(x)
    x = MaxPooling2D((2, 2), strides=(2, 2), name='pool1')(x)

    # Block 2
    x = Conv2D(128, (3, 3), use_bias=use_bias, activation='relu', padding='same', name='conv2_1')(
        x)
    x = Conv2D(128, (3, 3), use_bias=use_bias, activation='relu', padding='same', name='conv2_2')(
        x)
    x = MaxPooling2D((2, 2), strides=(2, 2), name='pool2')(x)

    # Block 3
    x = Conv2D(256, (3, 3), use_bias=use_bias, activation='relu', padding='same', name='conv3_1')(
        x)
    x = Conv2D(256, (3, 3), use_bias=use_bias, activation='relu', padding='same', name='conv3_2')(
        x)
    x = Conv2D(256, (3, 3), use_bias=use_bias, activation='relu', padding='same', name='conv3_3')(
        x)
    x = MaxPooling2D((2, 2), strides=(2, 2), name='pool3')(x)

    # Block 4
    x = Conv2D(512, (3, 3), use_bias=use_bias, activation='relu', padding='same', name='conv4_1')(
        x)
    x = Conv2D(512, (3, 3), use_bias=use_bias, activation='relu', padding='same', name='conv4_2')(
        x)
    x = Conv2D(512, (3, 3), use_bias=use_bias, activation='relu', padding='same', name='conv4_3')(
        x)
    x = MaxPooling2D((2, 2), strides=(2, 2), name='pool4')(x)

    # Block 5
    x = Conv2D(512, (3, 3), use_bias=use_bias, activation='relu', padding='same', name='conv5_1')(
        x)
    x = Conv2D(512, (3, 3), use_bias=use_bias, activation='relu', padding='same', name='conv5_2')(
        x)
    x = Conv2D(512, (3, 3), use_bias=use_bias, activation='relu', padding='same', name='conv5_3')(
        x)
...
def RESNET50(include_top=True, weights='vggface',
    ...
    x = Conv2D(
        64, (7, 7), use_bias=use_bias, strides=(2, 2), padding='same',
        name='conv1/7x7_s2')(img_input)
    x = BatchNormalization(axis=bn_axis, name='conv1/7x7_s2/bn')(x)
    x = Activation('relu')(x)
    x = MaxPooling2D((3, 3), strides=(2, 2))(x)

    x = resnet_conv_block(x, 3, [64, 64, 256], stage=2, block=1, strides=(1, 1), bias=use_bias)
    x = resnet_identity_block(x, 3, [64, 64, 256], stage=2, block=2, bias=use_bias)
    x = resnet_identity_block(x, 3, [64, 64, 256], stage=2, block=3, bias=use_bias)

    x = resnet_conv_block(x, 3, [128, 128, 512], stage=3, block=1, bias=use_bias)
    x = resnet_identity_block(x, 3, [128, 128, 512], stage=3, block=2, bias=use_bias)
    x = resnet_identity_block(x, 3, [128, 128, 512], stage=3, block=3, bias=use_bias)
    x = resnet_identity_block(x, 3, [128, 128, 512], stage=3, block=4, bias=use_bias)

    x = resnet_conv_block(x, 3, [256, 256, 1024], stage=4, block=1, bias=use_bias)
    x = resnet_identity_block(x, 3, [256, 256, 1024], stage=4, block=2, bias=use_bias)
    x = resnet_identity_block(x, 3, [256, 256, 1024], stage=4, block=3, bias=use_bias)
    x = resnet_identity_block(x, 3, [256, 256, 1024], stage=4, block=4, bias=use_bias)
    x = resnet_identity_block(x, 3, [256, 256, 1024], stage=4, block=5, bias=use_bias)
    x = resnet_identity_block(x, 3, [256, 256, 1024], stage=4, block=6, bias=use_bias)

    x = resnet_conv_block(x, 3, [512, 512, 2048], stage=5, block=1, bias=use_bias)
    x = resnet_identity_block(x, 3, [512, 512, 2048], stage=5, block=2, bias=use_bias)
    x = resnet_identity_block(x, 3, [512, 512, 2048], stage=5, block=3, bias=use_bias)
