...
class TCAV(object):
  ...
  @staticmethod
  def get_direction_dir_sign(mymodel, act, cav, concept, class_id, example, mode='gradient'):
    ...
    if mode == 'gradient':
      ...
      try:
        grad = np.reshape(mymodel.get_gradient(
          act, cav.bottleneck, example), (len(example), -1))
        # grad = np.reshape(mymodel.get_gradient(
        #   act, cav.bottleneck, example), -1)
      except Exception as e:
        print(example.shape)
        print(cav.bottleneck)
        print(act.shape)
        exit()
      dot_prod = np.dot(grad, cav.get_direction(concept))
    elif mode == 'jacobian':
      grad = np.reshape(mymodel.get_jacobian(
        act, cav.bottleneck, example), -1)
      dot_prod = np.dot(grad, cav.get_direction(concept))
    else:
      grad = np.reshape(mymodel.get_gradient(
  ...
  def compute_tcav_score(mymodel,
    ...
    """
    count = 0
    if mode != 'gradient' and mode != 'jacobian':
      class_id = mymodel.label_to_id(target_class)
    else:
      class_id = None
    counts = TCAV.get_direction_dir_sign(
        mymodel, class_acts, cav, concept, class_id, examples, mode=mode)
    return sum(counts) / float(len(class_acts))
    # if run_parallel:
    #   pool = multiprocessing.Pool(num_workers)
    #   directions = pool.map(
    #       lambda i: TCAV.get_direction_dir_sign(
    #           mymodel, np.expand_dims(class_acts[i], 0),
    #           cav, concept, class_id, examples[i], mode=mode),
    ...
    #         mymodel, act, cav, concept, class_id, example, mode=mode):
    #       count += 1
    #   return float(count) / float(len(class_acts))

  @staticmethod
  def get_directional_dir(
      mymodel, target_class, concept, cav, class_acts, examples, mode='gradient'):
    ...
    grad = np.reshape(
        mymodel.get_gradient(class_acts, cav.bottleneck, examples), (len(examples), -1))
  ...
  def __init__(self,
    ...
    # make pairs to test.
    # print(num_random_exp, len(random_concepts))
  ...
  def run(self, num_workers=10, run_parallel=False, overwrite=False, return_proto=False, mode='gradient'):
    ...
    now = time.time()
    if mode == 'cav':
        func = self._run_single_cav
    else:
        func = self._run_single_set
    if run_parallel:
      pool = multiprocessing.Pool(num_workers)
      for i, res in tqdm(enumerate(pool.imap(
          lambda p: func(
            p, overwrite=overwrite, run_parallel=run_parallel, mode=mode),
          self.params), 1)):
        tf.compat.v1.logging.info('Finished running param %s of %s' % (i, len(self.params)))
        results.append(res)
    else:
      for i, param in tqdm(enumerate(self.params)):
        tf.compat.v1.logging.info('Running param %s of %s' % (i, len(self.params)))
        results.append(func(param, overwrite=overwrite, run_parallel=run_parallel, mode=mode))
  ...
  def _run_single_set(self, param, overwrite=False, run_parallel=False, mode='gradient'):
    ...
    i_up = self.compute_tcav_score(
        ...
        run_parallel=run_parallel, mode=mode)
    val_directional_dirs = self.get_directional_dir(
        mymodel, target_class_for_compute_tcav_score, cav_concept,
        cav_instance, acts[target_class][cav_instance.bottleneck],
        activation_generator.get_examples_for_concept(target_class), mode=mode)
  ...
  def _run_single_cav(self, param, overwrite=False, run_parallel=False, mode='neither'):
    """Run TCAV with provided for one set of (target, concepts).
    ...
    acts_cav = []
    acts_example = []
    cav_direction = cav_instance.get_direction(cav_concept)
    for i in range(len(acts[target_class][cav_instance.bottleneck])):
        temp_example = activation_generator.get_examples_for_concept(target_class)[i]
        if len(temp_example.shape) == 3:
            temp_example = np.expand_dims(temp_example, 0)
        grads.append(np.reshape(mymodel.get_gradient(np.expand_dims(acts[target_class][cav_instance.bottleneck][i], 0), cav_instance.bottleneck, temp_example), -1))
        acts_cav.append(acts[target_class][cav_instance.bottleneck][i])
        acts_example.append(temp_example)

    np.savez(os.path.join(Config.DATA, 'embeddings', 'layers', 'tcav-{}-{}-{}'.format(cav_concept, target_class, cav_instance.bottleneck)),
        grads=grads, acts_cav=acts_cav, acts_example=acts_example, cav_direction=cav_direction)
    
    # vector = cav_instance.get_direction(cav_concept)
