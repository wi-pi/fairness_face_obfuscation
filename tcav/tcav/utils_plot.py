...
def plot_results(results, random_counterpart=None, random_concepts=None, num_random_exp=100,
  ...
  """
  csvfile = open('tcav/tcav_scores/{}'.format(outfile), 'w')
  ...
  for i, [bn, vals] in enumerate(plot_data.items()):
    print(index, i, bar_width, vals['bn_vals'])
  ...
  # print (plot_data)
  # set properties
  csvfile.write('{}\n'.format(plot_data.items()))
  csvfile.write('{}\n'.format(vals['significant']))
  csvfile.write('{}\n'.format(plot_concepts))
  ...
  plt.savefig(os.path.join(Config.DATA, 'embeddings', 'plots', 'tcav_{}.png'.format(outfile)))
