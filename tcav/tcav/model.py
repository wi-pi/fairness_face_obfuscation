...
class KerasModelWrapper(ModelWrapper):
  ...
  def __init__(
    ...
    self.import_keras_model()
    # self.names = np.load(get_file(labels_path,
    #   'https://github.com/rcmalli/keras-vggface/releases/download/v2.0/rcmalli_vggface_labels_v2.npy',
    #   cache_subdir=os.path.join(Config.ROOT, 'tcav')))
    self.labels = []
    with open(os.path.join(Config.ROOT, labels_path), 'r') as infile:
      for line in infile:
  ...
  def get_bottleneck_tensors(self):
    ...
    for layer in layers:
      print(layer.name, print(layer.output.shape))
      if 'input' not in layer.name:
        if len(self.only_layers) == 0 or layer.name in self.only_layers:
...
class KerasFaceModelWrapper(ModelWrapper):
  """ ModelWrapper for keras models
  ...
  def __init__(
    ...
    self.model = model
    self.only_layers = only_layers
    super(KerasFaceModelWrapper, self).__init__()
    self.import_keras_model(model_path)
    # self.labels = tf.io.gfile.GFile(labels_path).read().splitlines()

    # Construct gradient ops. Defaults to using the model's output layer
    # compute loss using distance from anchor to positive example
    self.image_shape = [160, 160, 3]

    graph = tf.compat.v1.get_default_graph()

    # with graph.as_default():
    # self.anchor = tf.compat.v1.placeholder(tf.float32, shape=[1, 160, 160, 3])
    self.anchor = tf.compat.v1.placeholder(tf.float32, shape=[None, 160, 160, 3])
    self.out_anchor = model(self.anchor)
    self.loss = tf.sqrt(tf.reduce_sum(tf.square(self.out_anchor - self.ends['prediction']), [1]))
  ...
  def _make_gradient_tensors(self):
    ...
    self.bottlenecks_gradients = {}
    for bn in tqdm(self.bottlenecks_tensors):
      self.bottlenecks_gradients[bn] = tf.gradients(
          ys=self.loss, xs=self.bottlenecks_tensors[bn])[0]
  ...
  def get_gradient(self, acts, bottleneck_name, example):
    """Return the gradient of the loss with respect to the bottleneck_name.
  ...
  def get_jacobian(self, acts, bottleneck_name, example):
    jacobian(self.bottlenecks_tensors[bottleneck_name], )
    return self.sess.run(self.bottlenecks_tensors[bottleneck_name], {})
