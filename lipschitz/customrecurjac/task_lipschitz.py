...
class task(object):
    ...
    def run_single(self, i):
        ...
        if args.dataset.startswith('vgg') or 'cifar' in args.dataset:
            # gradients = []
            # for i in tqdm(range(self.model.output.shape[1])):
            #     output_tensor = self.model.output[:,i]
            #     gradients.append(K.gradients(output_tensor, self.model.input)[0])
            # if sess is None:
            #     sess = K.get_session()

            # evaluated_gradients = []
            # for g in tqdm(self.gradients):
            #     evaluated_gradients.append(sess.run(g, feed_dict={self.model.input:inputs[i:i+1]}))
            # gradients = evaluated_gradients
            output_tensor = self.model.output[:,predict_label]
            g = K.gradients(output_tensor, self.model.input)[0]
            sess = K.get_session()
            obj_grad = (sess.run(g, feed_dict={self.model.input:inputs[i:i+1]}))
        else:
        ...
        if args.dataset.startswith('vgg') or 'cifar' in args.dataset:
            predictions = self.model.predict(inputs[i:i+1])
        else:
        ...
        self.outfile.write("[L1] model = {}, seq = {}, id = {}, true_class = {}, target_class = {}, info = {}, lipschitz_min = {:.5f}, lipschitz_max = {:.5f}, margin = {:.4f}, grad_norm = {:.4f}, time = {:.4f}\n".format(self.modelfile, i, self.true_ids[i], predict_label, target_label, self.img_info[i], self.min_lipschitz, self.max_lipschitz, margin, grad_norm, time.time() - start))
        # check results
        # if args.layerbndalg != "spectral":
    ...
    def summary(self, **kwargs):
        ...
        if self.force_label:
            ...
            self.outfile.write("\n\n[L0] model = {}, numimage = {}, {}, opnorm_global_lipschitz = {:.4f}\n\n".format(self.modelfile, self.n_points, lips_stats, lipschitz_const))
        else:
