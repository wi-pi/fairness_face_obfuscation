...
def get_weights_list_vgg(model, all_layers=True):
    ...
    if all_layers:
        for i, Ui in enumerate(U):
            # save hidden layer weights, layer by layer
            # middle layer weights: Ui
            output  = Ui.get_weights()
            if len(output) == 2:
                [weight_Ui, bias_Ui] = output
                if len(weight_Ui.shape) == 4:
                    weight_Ui = weight_Ui[0,0,:,:]
                if weight_Ui.shape[0] < 25000:
                    print("Hidden layer {} weight shape: {}".format(i, weight_Ui.shape))
                    print("Hidden layer {} bias shape: {}".format(i,bias_Ui.shape))
                    bias.append(np.ascontiguousarray(np.transpose(bias_Ui)))
                    weights.append(np.ascontiguousarray(np.transpose(weight_Ui)))
            elif len(output) == 1:
                weight_Ui = np.squeeze(output[0])
                if len(weight_Ui.shape) == 2 and (prev_size is None or prev_size == weight_Ui.shape[0]):
                    print("Hidden layer {} weight shape: {}".format(i, weight_Ui.shape))
                    print("Hidden layer {} bias shape: {}".format(i, weight_Ui.shape[1]))
                    weights.append(np.ascontiguousarray(np.transpose(weight_Ui)))
                    # bias.append(np.ascontiguousarray(np.transpose(np.ones(weight_Ui.shape[1], dtype=np.float32))))
                    bias.append(np.ascontiguousarray(np.transpose(np.zeros(weight_Ui.shape[1], dtype=np.float32))))
                    prev_size = weight_Ui.shape[1]
