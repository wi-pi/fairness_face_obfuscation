...
from setup_vgg import VGG_DATA
from mnist_cifar_models import NLayerModel, get_model_meta
from bound_base import get_weights_list, get_weights_list_vgg
...
if __name__ == "__main__":
    ...
    parser.add_argument('--exactsample',
                default="none",
                help='Target class label for robustness verification. Can be "least", "runnerup", "random" or "untargeted", or a number to specify class label.') 
    ...
    if 'imbalanced' in args.modelfile:
        balanced = 'imbal'
    elif 'balanced' in args.modelfile:
        balanced = 'bal'
    else:
        balanced = ''
    ...
    if args.exactsample != 'none':
        outpath = os.path.join(Config.DATA, 'fileio/lipschitz_constant-{}{}-{}-{}-{}-{}-{}-{}-{}.txt'.format(layer_str, args.dataset, balanced, args.layerbndalg, args.jacbndalg, args.lipsteps, args.eps, args.targettype, args.exactsample))
        exact_sample = int(args.exactsample)
    else:
        outpath = os.path.join(Config.DATA, 'fileio/lipschitz_constant-{}{}-{}-{}-{}-{}-{}-{}.txt'.format(layer_str, args.dataset, balanced, args.layerbndalg, args.jacbndalg, args.lipsteps, args.eps, args.targettype))
        exact_sample = None
    with open(outpath, 'w') as outfile:
        with tf.Session(config=config) as sess:
            ...
            elif args.dataset.startswith('vgg'):
                data = VGG_DATA(args.dataset.replace('vgg', ''), 10, args.targettype, modelfile.replace('models/', '').replace('-vgg16.hdf5', '').replace('simplenet_', '').replace('_generic_fourth.h5', ''), exact_sample)
            ...
            if args.dataset.startswith('vgg') or ('cifar' in args.dataset and 'simplenet' in modelfile):
                # print(np.argmin(model.predict(np.expand_dims(data.test_data[0], 0))))
                weights, biases = get_weights_list_vgg(model, all_layers=ALL_LAYERS)
                inputs, targets, true_labels, true_ids, img_info = generate_data(data, samples=data.test_labels.shape[0], total_images = args.numimage, targeted=targeted, random_and_least_likely = True, force_label = force_label, target_type = target_type, predictor=model.predict, start=args.startimage)
