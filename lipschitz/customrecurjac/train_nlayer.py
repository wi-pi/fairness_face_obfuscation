...
def train(data, file_name, params, num_epochs=50, batch_size=256, train_temp=1, init=None, lr=0.01, decay=0.0, momentum=0.9, activation="relu", activation_param=None, grad_reg = 0.0, dropout_rate = 0.0):
    ...
    if args.balanced == 'balanced':
        x = data.balanced_data
        y = data.balanced_data
    else:
        x = data.imbalanced_data
        y = data.imbalanced_labels
...
if __name__ == '__main__':
    ...
    if not args.modelfile:
        if args.balanced == 'balanced':
            file_name = args.modelpath+"/"+args.model+"_"+str(nlayers)+"layer_"+args.activation+"_"+args.layer_parameters[0]+'_balanced'
        else:
            file_name = args.modelpath+"/"+args.model+"_"+str(nlayers)+"layer_"+args.activation+"_"+args.layer_parameters[0]+'_imbalanced'
    else:
